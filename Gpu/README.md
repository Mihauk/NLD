# $\textcolor{lightblue} {Julia\ code\ using \ CUDA\ GPU's\ to\ time\ evolve\ a\ lattice\  gas\ diffusion\ model(with\ non-linearity).}$
...
- The argumens are optional so the position does not matters.
