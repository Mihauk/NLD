...
def realization():
    x = conf(ini_conf,n)
    #m_rho[:,0] = copy.deepcopy(x.state)
    m_rho_ft[:,0] += copy.deepcopy(x.ft)
    #qb2[0] = copy.deepcopy(x.ft[15])**2
    for i in range(t):
        x.update(n,np.random.randint(3))
        #qb2[i+1] = copy.deepcopy(x.ft[15])**2
        #m_rho[:,i+1] = copy.deepcopy(x.state)
        m_rho_ft[:,i+1] += copy.deepcopy(x.ft)
        #m_j[:,i] = copy.deepcopy(x.cur)
        #m_j_ft[:,i] = copy.deepcopy(x.cur_ft)
    m_rho_ft_sq = m_rho_ft*m_rho_ft
    avg = m_rho_ft/c
    var = (m_rho_ft_sq/c) -  avg*avg
    return avg, var
...
m_rho_ft_sq = np.zeros((l,t+1),dtype='complex')
